...
def Assert(obj, interface):
	...
	for name in i_dir:
		...
		if name not in o_dir:
			raise InterfaceException, 'Object %s does not match required interface %s: object does not implement %s \'%s\'' % (obj, interface, "method" if i_callable else "property", name)
		...
		if i_callable != o_callable:
			raise InterfaceException, 'Object %s does not match required interface %s: %s \'%s\' must be a %s' % (obj, interface, "method" if o_callable else "property", name, "method" if i_callable else "property")
		...
		if i_callable:
			...
			if o_required_count is not i_required_count or o_default_count is not i_default_count:
				raise InterfaceException, 'Object %s does not match required interface %s: method \'%s\' has a different positional or keyword argument count: found (%s), but expected (%s)' % (obj, interface, name, Interface._GetArgString(o_args), Interface._GetArgString(i_args))
